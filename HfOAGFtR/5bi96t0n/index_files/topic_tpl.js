...
if(t+e+100>n&&A.off(window,"scroll",a),!(G||-1==Z||Z>0&&n-t-e>500)){
...
var c="/mp/appmsg_comment?action=getcomment&scene="+Y.scene+"&__biz="+biz+"&appmsgid="+appmsgid+"&idx="+idx+"&comment_id="+S+"&offset="+Z+"&limit="+$+(window.send_time?"&send_time="+send_time:"");
try{
...
complete:function(){
G=!1,i(et.loading),A.off(window,"scroll",C);
}
});
}
}
...
error:function(e){
X.log("shit;"+e.status+";"+e.statusText),l(Y.addCommentErr,"type:ajaxerr;url:"+encodeURIComponent(o));
},
complete:function(){
...
s.new_appmsg=window.new_appmsg,m+=F.tmpl(x,s);
try{
...
e.preventDefault();
}),et.goback&&(A.on(et.goback,"tap",B),A.on(et.goback,"click",B)),window.__second_open__&&(A.on(et.input,"tap",function(){
i(et.fakebar);
...
e.preventDefault(),window.__second_open__?h():(!!O&&console.log("push comment"),
U.push("comment"));
