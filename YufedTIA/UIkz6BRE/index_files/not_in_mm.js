...
window.reportVid=[];
for(var l=Math.ceil(1e4*Math.random()),v=0,u=c.length;u>v;++v){
o=c[v];
var w=o.getAttribute("data-src")||"",_=o.className||"",f=o.getAttribute("src")||w;
if(!w||"#"==w){
...
x.length>1&&(w=x.join("&")+"#wechat_redirect");
}
}
if(f&&d.txVideoReg.test(f))if(s.isShowMpVideo()){
var y=n.getQuery("vid",w);
if(!y)continue;
...
}else d.videoIframeResizer.push(o);else if(w&&(w.indexOf("newappmsgvote")>-1&&_.indexOf("js_editor_vote_card")>=0||0==w.indexOf("http://mp.weixin.qq.com/bizmall/appmsgcard")&&_.indexOf("card_iframe")>=0||w.indexOf("appmsgvote")>-1||w.indexOf("mp.weixin.qq.com/mp/getcdnvideourl")>-1)){
if(window.is_transfer_msg&&!window.reprint_ticket&&w.indexOf(window.biz)<0){
p.push(o);
continue;
}
if(w=w.replace(/^http:/,location.protocol),_.indexOf("card_iframe")>=0){
var j=w.replace("#wechat_redirect",["&pass_ticket=",pass_ticket,"&scene=",source,"&msgid=",appmsgid,"&msgidx=",itemidx||idx,"&version=",version,"&devicetype=",window.devicetype||"","&child_biz=",biz,"&wxtoken=",window.wxtoken||""].join(""));
...
var z=w.indexOf("#wechat_redirect")>-1,O=["&uin=",uin,"&key=",key,"&pass_ticket=",pass_ticket,"&wxtoken=",window.wxtoken||""].join("");
reprint_ticket?O+=["&mid=",mid,"&idx=",idx,"&reprint_ticket=",reprint_ticket,"&source_mid=",source_mid,"&source_idx=",source_idx].join(""):_.indexOf("vote_iframe")>=0&&(O+=["&mid=",mid,"&idx=",idx,"&appmsg_token=",appmsg_token].join(""));
var j=z?w.replace("#wechat_redirect",O):w+O;
o.setAttribute("src",j);
}
-1==w.indexOf("mp.weixin.qq.com/mp/getcdnvideourl")&&!function(e){
e.onload=function(){
t(e);
};
}(o),o.appmsg_idx=v;
}
if(w&&w.indexOf("mp.weixin.qq.com/mp/getcdnvideourl")>-1&&b>0){
var I=b,A=3*I/4;
